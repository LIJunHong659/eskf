...
#define ESKF_STATE_DIM 11

/**
 * @brief 滤波器名义状态 (Nominal State)
 */
typedef struct {
    eskf_vec3_t pos;        // 位置 (m) - World Frame (ENU)，但z分量被约束
    eskf_vec3_t vel;        // 速度 (m/s) - World Frame (ENU)，但z分量被约束
 ...
 *        维度: 11x11
 *        顺序: pos(2, 只x,y), vel(2, 只x,y), theta(1, 只yaw), ba(3), bg(3)
 */
typedef struct {
